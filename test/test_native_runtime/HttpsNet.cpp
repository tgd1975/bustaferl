...
HttpResult HttpsNet::httpGet(const std::string &url, std::string &out) {
  out.clear();
  ...
  return {rc == CURLE_OK, static_cast<int>(status)};
}

HttpResult HttpsNet::httpPost(const std::string &url, const std::string &body,
  ...
  const std::string ct =
      content_type.empty() ? std::string{"application/json"} : content_type;
  const std::string ct_header = "Content-Type: " + ct;
