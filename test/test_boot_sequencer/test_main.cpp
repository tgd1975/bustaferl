...
void test_happy_path_returns_ok() {
    ...
    TEST_ASSERT_EQUAL(static_cast<int>(BootResult::Ok),
                      static_cast<int>(r));
}

void test_wifi_fail_first_attempt_retry_later() {
    FakeNet n;  n.will_connect = false;
    FakeClock c;
    BootConfig cfg;  cfg.max_retries = 5;
    auto r = runColdBoot(n, c, 0, cfg);
...
void test_wifi_fail_last_attempt_give_up() {
    ...
    TEST_ASSERT_EQUAL(static_cast<int>(BootResult::GiveUp),
                      static_cast<int>(r));
}

void test_ntp_fail_treated_like_wifi_fail() {
    FakeNet n;
    FakeClock c;  c.will_sync = false;
    BootConfig cfg;  cfg.max_retries = 5;
    auto r = runColdBoot(n, c, 4, cfg);
    TEST_ASSERT_EQUAL(static_cast<int>(BootResult::GiveUp),
                      static_cast<int>(r));
