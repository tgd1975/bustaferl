...
| Session E 2026-05-21 | §11.2–11.8 + Schritt 7 | **Option-A-Font-Stack (U8g2 Helvetica/Logisoso) ist akzeptabel.** Beobachtet im Host-Vergleich `docs/screenshots/host/` ↔ `docs/design_handoff_display/`: keine Y-/Baseline-Drift, kein Layout-Bruch durch Font-Metriken (V8 entschärft). Glyph-Stil weicht vom VT323/Silkscreen-Mock ab, aber bei 400×300 / 1-bit-Rendering nicht störend. **Option B (Custom-Bitmap-Fonts) wird nicht ausgelöst.** | [docs/v2-rollout/session-e-checklist.md](session-e-checklist.md) §11.8 Iteration 3, alle 7 Mockview-States verifiziert. |
| Session E 2026-05-21 | §11.2 + 11.8 | **Layout-Drift wurde initial unterschätzt.** Iteration 1 der §11.8-Sichtkontrolle hat nur Inhalts-Drift erfasst (Texte, Zahlen, Glyphen), nicht Geometrie (Spaltenausrichtung, Spacing, Marker-Größen). Iteration 2 (Claude-Design-Review) hat sieben Layout-Drifts (L1-L9) nachgereicht; davon erforderten **drei tatsächliche Code-Patches** (P0 EG-Separator `·`, P9 Time-Spalte rechtsbündig in `drawSlot`, P10 nutzt vorhandene `INTER_TIME_GAP_*`-Konstanten korrekt), die übrigen vier (P11/L7/L8/L9) waren bereits korrekt im Code — Iteration-1-Wahrnehmungsfehler. | Lehre für künftige §11.8-Reviews: **explizit pro Achse prüfen** (Spalten, Spacing, Marker-Größen, Linien-Stärken), nicht Frei-Vergleich. Memory [`feedback-pixel-compare-geometry`](../../.claude/projects/.../memory/feedback-pixel-compare-geometry.md). |
| Session E 2026-05-21 | §11.7 | **U+2026 (Ellipsis) nicht in `helvR14_te`.** Code bleibt bei `"lädt Fahrplan..."` (3 ASCII-Punkte) statt JSX-Designwert `"lädt Fahrplan…"`. Wechsel auf U+2026 ergibt leeres Glyph; Custom-Sprite wäre Overkill für kosmetische 1-Pixel-Drift. | Coverage-Check 2026-05-21 ergab leeres Render-Ergebnis bei `"\xE2\x80\xA6"` in `Fullscreen_Sub`-Font. |
| Session E 2026-05-21 | §11 generell | **HostCanvas = AdafruitGfxCanvas (Commit `b47ca14`).** Renderer-Validation läuft host-only via `pio test -e native -f test_native_mockview_dump` + `scripts/pgm-to-png.py`. **Geräte-Tests entfallen** für Display-Sichtkontrolle. §11.1 (Cold-Boot-Doppel-Refresh) bleibt der einzige Hardware-Schritt, ist aber Cycle-/Power-Test, kein Render-Test → deferred zu Session F/G. | Memory [`host-renderer-parity`](../../.claude/projects/.../memory/host-renderer-parity.md). |
