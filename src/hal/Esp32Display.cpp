...
void blit(Panel& panel, const uint8_t* fb) {
    panel.drawBitmap(0, 0, fb, EPD_WIDTH, EPD_HEIGHT, GxEPD_BLACK,
                     GxEPD_WHITE);
}
...
void bwFlash(Panel& panel) {
    panel.setFullWindow();
    panel.firstPage();
    do { panel.fillScreen(GxEPD_BLACK); } while (panel.nextPage());
    panel.firstPage();
...
struct Esp32Display::Impl {
    Panel panel{
        GxEPD2_420(/*CS=*/EPD_CS, /*DC=*/EPD_DC, /*RST=*/EPD_RST,
                   /*BUSY=*/EPD_BUSY)};
};
...
void Esp32Display::init() {
    impl_->panel.init(115200, true, 2, false);
}
