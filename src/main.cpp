...
std::string apiUrl() {
    ...
    char buf[64];
    snprintf(buf, sizeof(buf), "&rbl=%d&rbl=%d&rbl=%d",
...
bool fetchSnapshot(StreamSnapshot& out) {
    std::string body;
    if (!g_net.httpGet(apiUrl(), body)) return false;
...
void renderAndPush(const StreamSnapshot& snap, OverlayKind overlay,
    ...
    if (prev_valid) {
        prev_valid = g_store.loadFramebuffer(g_frame_prev.data(),
                                             Frame::bytes) == Frame::bytes;
    }

    time_t now = g_clock.now();
    RefreshConfig rc;
    RefreshDecision d = planRefresh(
        g_frame_prev.data(), g_frame_new.data(), prev_valid, now,
...
void doSleepOrLoop(const SleepDecision& sd, PersistedMeta& meta) {
...
void coldBootPath(PersistedMeta& meta) {
    ...
    BootResult r =
        runColdBoot(g_net, g_clock, meta.cold_boot_retries, bc);
...
void warmCyclePath(PersistedMeta& meta) {
    ...
    if (g_clock.now() - meta.last_ntp_sync > NTP_INTERVAL_S) {
        if (g_clock.ntpSync()) meta.last_ntp_sync = g_clock.now();
    }
    ...
    if (ok) {
        ...
        if (fh.isDead()) overlay = OverlayKind::FilterDead;
...
void loop() {
    // Active-phase polling: warmCyclePath set us up for lightSleep(POLL_INTERVAL_S)
    // and we get here when it returns. Just rerun the warm cycle.
    PersistedMeta meta = g_store.loadMeta();
    warmCyclePath(meta);
}
